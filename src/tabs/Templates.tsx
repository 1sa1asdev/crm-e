  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, width: '100%' }}>
      ...
      <div style={{ background: '#fff', color: '#111', width: '100%', padding: '10% 12%', fontFamily: 'Georgia, "Times New Roman", serif', fontSize: 11, lineHeight: 1.65, whiteSpace: 'pre-wrap', wordBreak: 'break-word', boxShadow: '0 2px 16px rgba(0,0,0,0.35)', minHeight: 320 }}>
        {filled || <span style={{ color: '#aaa', fontStyle: 'italic' }}>{t('tpl.previewEmpty')}</span>}
      </div>
    </div>
  )
}
...
function PlaceholderSidebar({ onInsert }: { onInsert: (p: string) => void }) {
  ...
  const groups = [
    {
      label: t('tpl.grpApplication'),
      items: [
        { tag: '{{company}}',      display: t('tpl.phCompany') },
        { tag: '{{role}}',         display: t('tpl.phRole') },
        { tag: '{{contact_name}}', display: t('tpl.phContactName') },
        { tag: '{{files}}',        display: t('tpl.phFiles') },
      ],
    },
    {
      label: t('tpl.grpProfile'),
      items: [
        { tag: '{{my_name}}',      display: t('tpl.phFirstName') },
        { tag: '{{my_last_name}}', display: t('tpl.phLastName') },
        { tag: '{{my_full_name}}', display: t('tpl.phFullName') },
        { tag: '{{my_email}}',     display: t('tpl.phEmail') },
        { tag: '{{my_phone}}',     display: t('tpl.phPhone') },
        { tag: '{{my_address}}',   display: t('tpl.phAddress') },
        { tag: '{{my_linkedin}}',  display: t('tpl.phLinkedin') },
      ],
    },
    ...(userLinks.length > 0 ? [{
      label: t('tpl.grpLinks'),
  ...
  return (
    <div style={{ width: 160, flexShrink: 0, borderLeft: '1px solid var(--color-edge)', padding: '20px 12px', display: 'flex', flexDirection: 'column', gap: 14, overflowY: 'auto' }}>
      <p className="m-0 text-[11px] text-lo/60">{t('tpl.insertHint')}</p>
      {groups.map(group => (
        <div key={group.label}>
...
function TplDialog({ open, initial, tplType, onClose, onSave }: {
  ...
  const dlgTitle = isCoverLetter
    ? (initial ? t('tpl.dlgEditLetter') : t('tpl.dlgNewLetter'))
    : (initial ? t('tpl.dlgEditEmail')  : t('tpl.dlgNewEmail'))
      ...
      <div style={{ display: 'flex', alignItems: 'stretch' }}>
        ...
        <form onSubmit={handleSubmit} style={{ flex: 1, minWidth: 0 }}>
          ...
          <label>{t('tpl.name')}
            <input name="name" required
              placeholder={isCoverLetter ? t('tpl.phNameLetter') : t('tpl.phNameEmail')}
              defaultValue={initial?.name ?? ''} key={`n-${initial?.id}`} />
          </label>
          {!isCoverLetter && (
            <label>{t('tpl.subject')}
              <input ref={subjectRef} name="subject" required
                placeholder={t('tpl.phSubject')}
          ...
          <menu>
            <button type="button" className="ghost" onClick={onClose}>{t('tpl.cancel')}</button>
            <button type="submit" className="primary">{t('tpl.save')}</button>
          </menu>
        </form>
        ...
        {isCoverLetter && (
          <div style={{ width: 320, flexShrink: 0, borderLeft: '1px solid var(--color-edge)', padding: 20, display: 'flex', flexDirection: 'column', gap: 10, background: 'var(--color-canvas)', overflowY: 'auto' }}>
            <div className="text-[11px] font-semibold text-lo/60 uppercase tracking-wide">{t('tpl.preview')}</div>
            <div className="text-[10px] text-lo/50 -mt-1">{t('tpl.previewNote')}</div>
...
export default function Templates() {
  ...
  const visible = state.templates.filter(tpl => (tpl.type ?? 'email') === activeTab)

  function saveTpl(data: Record<string, string>) {
    update(s => {
      if (data.id) {
        const idx = s.templates.findIndex(tpl => tpl.id === data.id)
  ...
  function deleteTpl(id: string) {
    ...
    if (!tpl || !confirm(t('tpl.deleteConfirm', { name: tpl.name }))) return
    update(s => { s.templates = s.templates.filter(x => x.id !== id) })
    toast(t('tpl.deleted'))
  }

  const tabBtn = (id: TplType, labelKey: string) => (
    <button key={id}
    ...
    <div className="p-6 max-w-[1200px] mx-auto">
      <div className="flex justify-between items-center mb-4 flex-wrap gap-3">
        <div className="flex gap-1">
          {tabBtn('email', 'tpl.emailTemplates')}
          {tabBtn('cover_letter', 'tpl.coverLetters')}
        </div>
        <button className="primary" onClick={() => { setEditing(null); setDialogOpen(true) }}>{t('tpl.new')}</button>
      </div>

      {visible.length === 0
        ? <p className="text-center text-lo p-10 bg-surface rounded-lg border border-dashed border-edge">
            {activeTab === 'email' ? t('tpl.noEmail') : t('tpl.noLetters')}
          </p>
        : (
          <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-3">
            {visible.map(tpl => (
              <div key={tpl.id} className="bg-surface border border-edge rounded-lg p-[14px]">
                <h3 className="m-0 mb-1.5 text-sm font-semibold">{tpl.name}</h3>
                {tpl.subject && <div className="text-lo text-xs mb-2.5">{tpl.subject}</div>}
                <div className="text-[13px] text-lo max-h-[80px] overflow-hidden mb-2.5 whitespace-pre-wrap">{tpl.body}</div>
                <div className="flex gap-1.5">
                  <button className="px-[10px] py-1 text-xs" onClick={() => { setEditing(tpl); setDialogOpen(true) }}>{t('tpl.edit')}</button>
                  <button className="px-[10px] py-1 text-xs" onClick={() => deleteTpl(tpl.id)}>{t('tpl.delete')}</button>
                </div>
              </div>
            ))}
          </div>
